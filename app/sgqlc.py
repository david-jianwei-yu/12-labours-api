...
class SimpleGraphQLClient:
    ...
    def get_queried_result(self, item, SUBMISSION, id=None, queue=None):
        if item.node == None:
        ...
        try:
            result = SUBMISSION.query(query)["data"]
            if queue != None and result[item.node] != []:
                queue.put({id: result[item.node]})
            return result
